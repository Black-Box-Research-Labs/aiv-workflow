...
function prSummaryIssues(body, opts = {}) {
  ...
  // #199: a stale-base target whose deployed aiv.guard predates v2.2 acceptance requires the LITERAL
  // '# AIV Verification Packet (v2.1)' header — a v2.2 header FAILS its required-section check (the newer
  // aiv.guard accepts either, older ones only v2.1). When the spec pins packetHeaderVersion:"v2.1" the LOCAL
  // gate must MIRROR the target (require v2.1, reject v2.2) so pr-summary regenerates a v2.2 body DOWN to v2.1
  // instead of declaring it "perfect". This is the exact drift a fix could not self-correct: the harness gate
  // accepted v2.2 while the target CI rejected it, so pr-summary never rewrote the body. Default: accept both.
  const headerAlts = opts.headerVersion === "v2.1"
...
async function prSummaryLoop(repo, pull, cwd, finding, spec) {
  ...
  for (let round = 1; round <= CAP; round++) {
    ...
    const hdrVer = (spec && spec.packetHeaderVersion) === "v2.1" ? "v2.1" : "v2.2";   // #199: target-required packet header version (stale-base aiv.guard rejects v2.2)
    const checkOpts = { repo, headShaShort: headShort, intentSubstr, title: pr.title, findingDesc, provenanceTag: provTag, headerVersion: spec && spec.packetHeaderVersion };  // #20: title audited; #36: provenance anchor declared; #199: header version
    ...
    const newIssues = prSummaryIssues(newBody, { repo, headShaShort: headShort, intentSubstr, title: newTitle, findingDesc, provenanceTag: provTag, headerVersion: spec && spec.packetHeaderVersion });
...
[3] FIX APPROACH: RUNNER READS SAMPLER-PROVIDED KEYS
  ...
  // #199: target-required packet header version — a stale-base aiv.guard rejects a v2.2 header (requires literal v2.1)
