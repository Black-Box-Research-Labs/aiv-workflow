...
> (`─────── name ───────`) and in ~108 numbered-learning comments (`#NN`). This
...
- **Packet** — a Layer-2 "AIV Verification Packet": a structured markdown document
  ...
  Evidence, one section per evidence class, Claim Verification Matrix, Verification
...
- **Evidence classes A–F** — the kinds of evidence a packet carries. **Canonical
  ...
  **required** rises with the risk tier (A+B always; E from R1; C from R2; D+F from
  R3). ⚠ **Divergence you must know:** `fix_pipeline.mjs` labels its **lint/type**
  evidence "Class D" (INVARIANT #9; `completePacketClasses`), but
  canonical D is **Differential** — lint/type isn't an AIV evidence class at all
  (the spec's example packet puts it in a separate "Code Quality" section). It
  survives only because `aiv check` validates section *structure*, not class
  *semantics*. See §12.
...
- **The aiv lifecycle** — the CLI ceremony a code-writing stage performs:
  ...
  validates a packet's *structure* (8-stage pipeline), `aiv audit` its *content*
  quality. When a weak model fumbles this ceremony, the harness performs it (§8).
...
`STAGES` is the canonical 14-step list. `driveSpine` walks it
with a **resume cursor**: `state.findings[fid].stages` records which steps are done,
and each `isDone(st)` check skips completed steps, so a killed drive resumes where
it stopped. The stages, in execution order:
...
| 2–3 | **plan ↔ check-drift (Loop #1)** | `driveSpine` (inline) | convergence loop |
| 4 | ground (provision venv + capture baseline) | `provisionEnv` | mechanical |
...
| 9–12 | **back-half convergence** | `driveSpine` (inline) | convergence loop |
...
- **`--selftest`** — zero-API: 408 assertions over the pure predicates + stage
  structure (grows as fixes land; 0 failed is the gate) (`selftest`).
- **`--dry-run`** — zero-API end-to-end over **fixture** gate outputs
  (`drive`, `dryFixtures`). ⚠ See the FIX-05 caveat in §12: this
  exercises `loopPlan`/`loopImpl`, which are **not** the loops the spine runs.
...
- **Extract** (`extractMachineBlock`): pull the *last* `## Machine-checkable
  data` fenced block from the artifact; fall back to brace-slicing; then to the
  agent's prose response (`#100` narration recovery); then to `scavengeBlock`
  if the model wrote to a hallucinated path.
...
| `prove_it_manifest` | `gateProveIt` | `unverified_count==0`, all claims PASS |
...
fatal/config · `3` HALT · `4` gate-not-converged (only via `--run-stage`) · `5`
...
**Loop #1 — plan ↔ check-drift** (inline in `driveSpine`). The plan
...
**Loop #2 — the back-half** (inline). Described in §2.

⚠ **Do not confuse these with `loopPlan` / `loopImpl`.** Those are a
*separate, simpler* abstraction used only by `drive()` (the dry-run/selftest
fixture walker) and the selftest — they are **never called by `driveSpine`**
(verify: `grep -nE '\bloop(Plan|Impl)\(' orchestration/src/fix_pipeline.mjs`). The tested loops and
the shipped loops are parallel implementations. See FIX-05 in §12.
...
- **FIX-01 (HIGH):** `prove-it` is the one gate in the fix-reality chain that is
  **self-attested** — `gateProveIt` judges the agent's manifest; the harness
  never independently re-runs the RED test against the cited baseline. This isn't
  just a re-execution gap I noticed: **`prove-it`'s own `SKILL.md` makes bar #4 of a
  valid artifact "an independent assessor (not you) confirms it on the artifact that
  will SHIP"** — and the pipeline provides no such assessor, so it omits a check the
  skill it runs explicitly requires. Compounded with `verify-finding` defaulting to
  *proceed* on `inconclusive` (`gateFindingVerified`), a finding whose defect was never
  harness-confirmed can ship on self-attested evidence. *If you harden one thing,
  harden this:* re-execute the baseline demo harness-side, mirroring
  `verify-finding`'s re-run.
- **FIX-05 (MEDIUM):** the tested convergence loops (`loopPlan`/`loopImpl`) are
  **not** the shipped loops (`driveSpine` inlines its own). A green `--dry-run`
  certifies the abstraction, not the production back-half. Mutating the `stable`
  predicate produces no red test today.
- **FIX-02 (MEDIUM):** baseline caches (§9) are WORK-global, repo/finding-agnostic,
  early-returned on existence, and not cleared by fresh-start hygiene.
  Reusing one clone across findings/repos silently reuses a stale
  baseline. Safe usage is one-clone-per-finding; nothing enforces it.
- **FIX-04 (LOW):** the dual HALT mechanism in §9 — live-loop halts don't set
  `status:"halted"`.
- **FIX-03 (LOW / doc-gap):** the `#124`/`#125`/`#29` terminators convert subjective
  blocking signals to PASS-with-advisory routed to H2. Intentional, but it means
  the "0 load-bearing unverified" property depends on the human reading
  `audit_churn_advisories.md`. Make that channel load-bearing in the H2 contract.
- **AIV-DIV-01 (decide):** `fix_pipeline.mjs` labels its **lint/type** evidence
  "Class D" (INVARIANT #9; `completePacketClasses`), but in the
  canonical AIV spec **Class D is Differential** (change impact — API/state/config),
  and lint/type is not an evidence class at all (the spec's own example packet puts
  it in a separate "Code Quality" section). The mislabel survives because
  `aiv check` validates section *structure*, not class *semantics*, and `aiv audit`
  doesn't semantically check D. Consequence: this pipeline's packets advertise
  Differential evidence they don't contain, and omit the Differential evidence a
  reviewer may expect. This is a **decision, not a confirmed bug**: either relabel
  (move lint to a Code-Quality section; supply real Differential evidence for D) or
  document the local convention. See `AIV_PRIMER.md` for the canonical definitions.
...
- **Add a stage:** add a `LIVE_STAGES` entry (§5 field contract), add it to
  ...
  for the predicate (the suite tests predicates, not loops — §12 FIX-05).
...
- **`#NN` comments** are numbered learnings — each documents a specific observed
  ...
  `#NN`.** ~108 of them; they are the real changelog.
...
```json
{
  ...
  "control_flow": {
    "stages_list": "STAGES",
    "loop1_plan_checkdrift": "driveSpine",
    "loop2_backhalf": "driveSpine",
    "tested_loops_NOT_shipped": ["loopPlan", "loopImpl"],
  ...
  "deterministic_overrides": ["crActionableCount", "auditFixLoop", "gateOrReview_no_vacuity"],
  "ceremony_subsystem": {
  ...
  "exit_codes": { "0": "success/preview", "2": "fatal/config", "3": "HALT", "4": "gate-not-converged(--run-stage)", "5": "REFUTED" },
  ...
  "sharp_edges": ["FIX-01 prove-it self-attested", "FIX-05 tested!=shipped loops", "FIX-02 WORK-global baselines", "FIX-04 dual halt mechanism", "FIX-03 advisory downgrade"],
